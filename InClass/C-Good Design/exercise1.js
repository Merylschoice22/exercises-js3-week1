...
const tax1150L = 0.1;
const taxST = 0.5;
const taxDefault = 0.8;
const incomeTaxes = [1000, 580];
const studentLoanFixed = 17775;
const studentLoanInterest = 0.09;

/*** CALCULATE NATIONAL INSURANCE FUNCTION ***/

function calculateNationalInsurance(taxCode, salary) {
  if (taxCode === "1150L") {
    nationalInsurance = salary * tax1150L;
  } else if (taxCode === "ST") {
    nationalInsurance = salary * taxST;
  } else {
    nationalInsurance = salary * taxDefault;
  }
...
function calculateIncome(salary, taxCode, incomeTaxes) {
  ...
  // STUDENT LOAN
  var studentLoan = (salary - studentLoanFixed) * studentLoanInterest;
